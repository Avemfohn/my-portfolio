...
import { X, Github, ExternalLink } from 'lucide-react'; // İkonlar için (yoksa normal text kullan)
...
const ProjectsList = ({ projects, error }: ProjectsListProps) => {
  ...
  // Hangi projenin seçili olduğunu tutan state
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {projects.map((project, index) => (
              <ProjectCard
                ...
                project={project}
                index={index}
                openModal={setSelectedProject} // Fonksiyonu gönderdik
              />
            ))}
          </div>
        ) : (
...
// --- MODAL BİLEŞENİ (Aynı dosyanın altına ekleyebilirsin) ---
const ProjectModal = ({ project, closeModal, language }: { project: Project, closeModal: () => void, language: 'tr' | 'en' }) => {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      onClick={closeModal} // Arka plana basınca kapat
    ...
    >
      <motion.div
        ...
        onClick={(e) => e.stopPropagation()} // İçeriğe basınca kapanmasın
      ...
      >
        ...
        <div className="p-8">
          ...
          {/* Teknolojiler */}
          <div className="mb-8">
            <h4 className="text-sm font-semibold text-slate-400 uppercase mb-3 tracking-wider">Kullanılan Teknolojiler</h4>
          ...
          <div className="flex gap-4 pt-4 border-t border-slate-700">
            {project.github_link && (
              <a href={project.github_link} target="_blank" rel="noreferrer" className="flex items-center gap-2 px-6 py-3 bg-slate-700 hover:bg-white hover:text-black text-white rounded-lg transition-all font-medium">
                <Github className="w-5 h-5" /> Kaynak Kod
              </a>
            )}
            {project.live_link && (
              <a href={project.live_link} target="_blank" rel="noreferrer" className="flex items-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-all font-medium">
                <ExternalLink className="w-5 h-5" /> Canlı Proje
              </a>
            )}
          </div>
        </div>
